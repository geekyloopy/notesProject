...
module.exports = {
    register: async function(req,res){
        ...
        await Users.create({uname: uname,password: password}).exec(function(err){
            if(err){
    ...
    list: async function(req,res){
        await Users.find({}).exec(function(err,userList){
            if(err){
                res.send(500,{err: 'Database error..!!'});
            }
    ...
    login: async function(req,res){
        ...
        Users.findOne({uname: loginUname}).exec(function(err,loginUser){
    ...
    updatePassword: async function(req,res){
        ...
        var updatedRecord= await Users.updateOne({uname: loginUser.uname}).set({password: loginUser.password});
        if(updatedRecord){
    ...
    listByUser: async function(req,res){
        const loginUser=req.param('uname');
    ...
    getPassword: async function(req,res){
        const loginUser=req.param('uname');
        var matchedRecord=await Users.findOne({uname: loginUser});
        console.log(matchedRecord.password);
        if(matchedRecord){
            return res.status(200).json({data: matchedRecord.password});
        }
        else{
            return res.status(400).json({message: 'Bad request from client..!!'});
        }
